...
function readEnv(name: string): string | undefined {
  ...
  // Dynamic key so Vite cannot inline this as `undefined` at build time.
  const value = process.env[name];
  return value && value.trim() ? value.trim() : undefined;
}

function currentDatabaseUrl(): string | undefined {
...
export const dbSource: DbSource = currentDatabaseUrl() ? "neon" : "pglite";
...
/** Neon / Supabase / any hosted Postgres: SSL on unless the URL is loopback. */
export function pgPoolOptions(url: string, max = 5) {
  const loopback =
    /localhost|127\.0\.0\.1/i.test(url) && !/sslmode=require/i.test(url);
  return {
    connectionString: url,
    max,
    ssl: loopback ? undefined : { rejectUnauthorized: false as const },
  };
}

function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    // Regular Postgres driver: node-postgres (`pg`) — Neon pooled, Supabase
    // session-mode pooler, or any DATABASE_URL. One pool per process; warm
...
/** Apply migrations/*.sql to hosted Postgres. Same files as scripts/migrate.mjs. */
  ...
  for (const { name, path } of pendingMigrations(Object.keys(migrations), applied)) {
    ...
    try {
      ...
      if (/already exists|duplicate/i.test(msg)) {
        ...
        console.log(`[db] skipped ${name} (${msg})`);
      } else {
        throw err;
      }
    } finally {
...
async function createSql(): Promise<Sql> {
  ...
  const source: DbSource = currentDatabaseUrl() ? "neon" : "pglite";
 ...
 *
 * - **Postgres** (`DATABASE_URL`): open the pool and apply pending
 *   `migrations/*.sql` on first use (also used on Vercel).
